...
EVENT_HUB_NAME = spark.conf.get("EVENT_HUB_SHIPMENTS_NAME")
EVENT_HUB_CONSUMER_GROUP = spark.conf.get("EVENT_HUB_SHIPMENTS_CONSUMER_GROUP")
...
@dp.table(
    name=f"{catalog_name}.{bronze_schema}.shipment_events_raw",
    comment="Raw shipment_created events ingested from Azure Event Hubs.",
)
def shipment_events_raw():
    ...
    return (
        ...
        .withColumn("_event_type", lit("shipment_created"))
